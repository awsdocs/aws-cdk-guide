...
{aws} CDK collects telemetry data on CLI usage to provide aggregate usage patterns and error frequencies that will help us identify widespread issues affecting the CDK CLI user base. In CDK CLI version 2.1100.0 and up, customers will be opted in to CDK CLI usage data collection and reporting by default. To learn what data is collected and how to opt out of CDK CLI telemetry, see xref:cli-telemetry[Configure {aws} CDK CLI telemetry].
