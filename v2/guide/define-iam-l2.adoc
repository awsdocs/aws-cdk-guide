...
TIP: Grant methods also work with L1 constructs through Grants classes. For more information, see xref:facades[Facades].

The following is an example that defines permissions between an {aws} Lambda function and Amazon Simple Storage Service (Amazon S3) bucket. Here, the `grants.read` method of the Bucket L2 construct is used to define these permissions:
...
When you use grant methods of L2 constructs to define permissions between resources, the {aws} CDK will create roles with least privilege policies based on the method you specify. As a security best practice, we recommend that you use the method that applies only the permissions that you require. For example, if you only need to grant permissions for a Lambda function to read from an Amazon S3 bucket, use the `grants.read` method instead of `grants.readWrite`.
...
However, we recommend that you use the methods in the `grants` property when available.
