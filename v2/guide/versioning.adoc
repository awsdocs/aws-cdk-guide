...
* **Bug-free programs**
+
We will make sure that correct programs continue to work with newer versions, but this guarantee does not extend to programs that are formally incorrect but happen to compile and run successfully due to quirks. For example, if your program relies on https://www.typescriptlang.org/docs/handbook/type-compatibility.html[TypeScripts structural typing rules] to pass objects of a certain type in a place where these aren't expected, or if your program synthesizes successfully but produces a CloudFormation template that doesn't deploy correctly or doesn't achieve the stated intent of the API, we will consider any changes that make your program fail to compile or run as "bug fixes" instead of "breaking changes".
