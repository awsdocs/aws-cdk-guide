...
[#develop-customize-mixins]
== Use Mixins to add features to L1 constructs

Mixins let you add high-level features to L1 constructs without needing a full L2 construct.
This is useful when an L2 construct is not available for a service,
or when you want to use L1 constructs with specific features that are normally only available through L2 constructs.
...
Mixins are available through the `mixins` namespace of each service module.
You can combine Mixins with xref:develop-customize-facades[Facades] to also get permissions and other integrations on L1 constructs.
For more information about Mixins, see xref:mixins[Mixins].

[#develop-customize-facades]
== Use Facades for integrations on L1 constructs

Facades are resource-specific classes that provide integrations — such as permissions, metrics, and reflections — for both L1 and L2 constructs.
The most common Facades are Grants classes, which provide intent-based permission methods.
...
For more information about Facades, see xref:facades[Facades].
...
TIP: If you only need permissions or other integrations on an L1 construct, you don't need to wrap it in an L2. Use xref:facades[Facades] like Grants classes directly on the L1 instead — for example, `BucketGrants.fromBucket(cfnBucket).read(role)`.
