...
[TIP]
====
You can proactively prevent logical ID changes when you extract resources into higher-level constructs.
By using `Default` as the construct ID for the primary resource in a wrapper construct, the logical ID remains unchanged.
For more information, see xref:identifiers-logical-id-heuristics[Logical ID path component heuristics].
====
