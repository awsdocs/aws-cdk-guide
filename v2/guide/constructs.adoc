...
* **Object references**: Pass construct (L1 or L2) objects directly
...
The following example demonstrates both approaches when creating a Lambda function that references an IAM role:
...
const myFunction = new lambda.CfnFunction(this, "HelloWorldFunction", {
  runtime: 'nodejs24.x',
  role: role, // Pass the role object directly
  // role: role.roleArn, // Or pass the ARN explicitly
  handler: 'index.handler',
  code: {
    zipFile: `
    exports.handler = async function(event) {
...
// myFunction.role will contain the role ARN
----
...
my_function = _lambda.CfnFunction(self, "HelloWorldFunction",
    runtime="nodejs24.x",
    role=role,  # Pass the role object directly
    # role=role.role_arn,  # Or pass the ARN explicitly
    handler="index.handler",
...
# my_function.role will contain the role ARN
----
...
CfnFunction myFunction = CfnFunction.Builder.create(this, "HelloWorldFunction")
    .runtime("nodejs24.x")
    .role(role)  // Pass the role object directly
    // .role(role.getRoleArn())  // Or pass the ARN explicitly
    .handler("index.handler")
...
// myFunction.getRole() will contain the role ARN
----
